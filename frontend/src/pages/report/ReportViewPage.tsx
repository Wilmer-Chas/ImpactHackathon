...
export function ReportViewPage() {
  ...
  async function onGenerate() {
    ...
    setError(null);
    try {
      const data = await generateMonthlyReport(period);
      setReport(data);
      navigate(`/report/${data.period}`, { replace: true });
    } catch (err) {
  ...
  if (!report) {
    return (
      <div className="max-w-6xl mx-auto p-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Report View</h1>
  ...
  const chartData = report.performance.map((p) => ({
    name: p.monthLabel,
    value: p.value,
    fill: p.isAnomaly ? "#EF4444" : "#6366F1",
  }));
  const target = report.performance[0]?.target ?? 90;
  const anomaly = report.performance.find((p) => p.isAnomaly);
  ...
  return (
    <div className="max-w-6xl mx-auto p-8">
      ...
      <div className="bg-brand-blue text-white rounded-xl p-6 mb-8 shadow-sm flex justify-between items-start gap-4">
        <div>
          <h1 className="text-2xl font-bold mb-1">{report.title}</h1>
          <p className="text-sm text-white/80">{report.subtitle}</p>
        </div>
        <button
          type="button"
          onClick={() => void onGenerate()}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <div className="bg-green-50 text-green-700 text-xs font-semibold px-3 py-1.5 rounded mb-3 flex justify-between">
              <span>NEW</span>
              <span>{report.riskRegister.new.length}</span>
            </div>
            <div className="space-y-3">
              {report.riskRegister.new.map((r) => (
          ...
          <div>
            <div className="bg-blue-50 text-blue-700 text-xs font-semibold px-3 py-1.5 rounded mb-3 flex justify-between">
              <span>RESOLVED</span>
              <span>{report.riskRegister.resolved.length}</span>
            </div>
            <div className="space-y-3">
              {report.riskRegister.resolved.map((r) => (
                <div
                  key={r.id}
